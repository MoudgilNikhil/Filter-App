...
const displayFucntion = (f, airlines) => {
  ...
  document.getElementById("flights").innerHTML = flightsArray
    .map(
      (flight) =>
        `<div class="Details" id="detailBlock"=>
                        <div class="direction">${getFlightCompany(flight.ValidatingAirlineCode, airlines)}
                        </div>
                        <div class="direction">Amount: ${
                          flight.AirItineraryPricingInfo.ItinTotalFare.TotalFare.Amount} ${flight.AirItineraryPricingInfo.ItinTotalFare.TotalFare.CurrencyCode}
                        </div>
                        <div class="ArrDep">
                            <div>Arrival Time
                            </div>
                            <div>${getDate(flight.OriginDestinationOptions[0].FlightSegments[0].ArrivalDateTime)} ${getTime(flight.OriginDestinationOptions[0].FlightSegments[0].ArrivalDateTime)}
                            </div>
                        </div>
                        ...
                        <div class="ArrDep">
                            ...
                            <div>${getDate(flight.OriginDestinationOptions[0].FlightSegments[0].DepartureDateTime)} ${getTime(flight.OriginDestinationOptions[0].FlightSegments[0].DepartureDateTime)}</div>
                        </div>
          </div>`
    )
    .join("");
...
flights= ["Air India", "Malmo Aviation", "Turkish Airlines", 'Austral Lineas Aereas', 'British Airways', 'American Airlines','Etihad Airways']
var elms = document.querySelectorAll("[id='inputs']");
for(var i = 0; i < elms.length; i++) {
